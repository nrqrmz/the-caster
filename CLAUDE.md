...
- `regions.js` — the campaign: `REGIONS` keyed by id (`fire`/`water`/`air`/`earth`/`castle`), built by `makeBranch`/`makeCastle`. A region has `element`, `grantsSkill`, and an array of `levels`. A level is built by `makeLevel` (`data/levelBuilder.js`) from a `kind` preset (`basic`/`intermediate`/`levelboss`/`temple`; `pretemple` is castle-only) into an ordered `phases` array. `REGION_ORDER`, `CASTLE_ID`, `REQUIRED_ELEMENTS` drive the map and gating.
  - **Elemental branches are 8 levels, one boss per level** (standard since 2026-06-10): nv1–3 `basic`, nv4–6 `intermediate` (one miniboss each), **nv7 `levelboss` = a dedicated boss-only level** (Fire's three-sisters trio + lava triangle; in general the level boss / multi-boss + its arena gimmick), nv8 `temple` (temple boss). Never stack two bosses in one level. When adding a new level `kind`, wire it everywhere a kind is consumed: `KIND_PHASES`/`DEFAULT_REWARD` (`levelBuilder`), `KIND_LABEL` (`BranchScene`), `GOLD_BASE`/`GOLD_PAR_MS` (`Economy`), and `BASE_CURVE` length (`tuning`, one entry per level depth). The **Castle** is the exception — own structure (5 levels, uses `pretemple`), evolving in its own spec.
...
- Sprite recipes may declare `static: true` (a single front frame; never flipped unless the recipe also declares `faces: true`, for a static side-profile sprite drawn facing right — pair it with the enemy's `facePlayer`, e.g. `can_lava`) and `body: {x, y, size?}` (a `size×size` body box — default 32; `size = radius*2` renders at scale 1 — inside a larger `gridW×gridH` canvas, so halos/torches/wings overflow the box while the hitbox stays sized to the body). Normal enemies are sized via `Enemy#applyDisplay()` / `displayFor` (`src/systems/enemyDisplay.js`), never `setDisplaySize(radius * 2, …)`. Parts generated from reference images (e.g. `partsFireBasics.js` / `partsFireAdvanced.js` / `partsFireBosses.js` by `tools/gen-fire-basics.mjs` / `tools/gen-fire-advanced.mjs` / `tools/gen-fire-bosses.mjs`, sharing `tools/lib/sheetParts.mjs`; bosses use `fitSide` for an exact longest side) are never hand-edited.
